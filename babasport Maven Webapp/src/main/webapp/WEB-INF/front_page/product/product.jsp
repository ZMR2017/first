	...
	<div class="r wr">
		...
		<h2 class="h2 h2_filter mt"><em title="商品筛选">商品筛选</em><cite><a href="javascript:void(0);" id="filterRest" title="重置筛选条件">重置筛选条件</a></cite></h2>
			<ul class="uls filter">
				...
				<li><label>已选条件：</label>
				<p class="sel">
					<c:forEach items="${query }" var="enrty">
						<a href="javascript:void(0);">
						<em>${enrty.key }：</em>${enrty.value }
				...
				<li><b>品牌：</b><p>
					...
					<c:forEach items="${brands }" var="brand">
						<a onclick="window.location.href='/product/display/list.shtml?brandId=${brand.id}&brandName=${brand.name}&typeId=${typeId}&typeName=${typeName}'" href="javascript:void(0);" title="${brand.name }">${brand.name }</a>
					</c:forEach>
				</p></li>
				</c:if>
				<li><b>价格：</b><p>
